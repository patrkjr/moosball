...
export async function getPlayerProfile(
  ...
  const entry = leaderboard.find((row) => row.id === id);
...
export async function getPlayerGames(
  ...
  return db
    ...
    .where(
      or(
        eq(games.team1Player1Id, playerId),
        eq(games.team1Player2Id, playerId),
        eq(games.team2Player1Id, playerId),
        eq(games.team2Player2Id, playerId),
      ),
    )
